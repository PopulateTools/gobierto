...
window.GobiertoAdmin.ProcessPollsController = (function() {
  ...
  function _addEditQuestionModalBehaviors() {
    ...
    $(".open_poll_modal").magnificPopup({
      ...
      closeMarkup:
        "<button title='Close (Esc)' type='button' class='mfp-close close_poll_modal' data-behavior='cancel_save_question'>×</button>",
      ...
      callbacks: {
        ...
        open: function() {
          ...
          var closestLocale = $("#edit_question_" + questionIdx)
            .find(".globalized_fields")
            .find("a.selected")
            .text()
            .trim()
            .toLowerCase();
  ...
  function createNewQuestionSummary($existingQuestions, questionIdx) {
    ...
    $newQuestionSummary.find("a[data-behavior=edit_record]").attr("href", "#edit_question_" + questionIdx);
  ...
  function createNewQuestionModal(questionIdx) {
    ...
    $newQuestionDestroyInput
      .attr("name", "poll[questions_attributes][" + questionIdx + "][_destroy]")
    ...
    $newQuestionOrderInput
      .attr("name", "poll[questions_attributes][" + questionIdx + "][order]")
    ...
    $newQuestionModal.find(".option").each(function() {
      ...
      if (id && id.match(/poll_questions_attributes_scaffold_answer_type_.*/)) {
        ...
        $input.attr("id", newId).attr("name", "poll[questions_attributes][" + questionIdx + "][answer_type]");
  ...
  function handleConfirmEditAnswerTemplate(e) {
    ...
    var $answerInput = $editAnswerWrapper.find(
      "input[name='" + generateNameForAnswerTemplateAttribute(questionIdx, answerIdx, "text") + "']"
    );
  ...
  function handleAddAnswerTemplate(e) {
    ...
    $newAnswerWrapper.addClass("answer_template").removeClass("answer_template_scaffold");
    ...
    $newAnswerEditPanel.attr("id", "question_" + questionIdx + "_edit_answer_" + newAnswerIdx).show();
    ...
    $newAnswerTextInput
      .attr("name", generateNameForAnswerTemplateAttribute(questionIdx, newAnswerIdx, "text"))
    ...
    setTimeout(function() {
      $("#" + generateIdForAnswerTemplateAttribute(questionIdx, newAnswerIdx, "text")).focus();
    }, 50);
    ...
    $newAnswerImageInput
      .attr("name", generateNameForAnswerTemplateAttribute(questionIdx, newAnswerIdx, "image_file"))
    ...
    $newAnswerOrderInput
      .attr("name", generateNameForAnswerTemplateAttribute(questionIdx, newAnswerIdx, "order"))
    ...
    $newAnswerDestroyInput
      .attr("name", generateNameForAnswerTemplateAttribute(questionIdx, newAnswerIdx, "_destroy"))
