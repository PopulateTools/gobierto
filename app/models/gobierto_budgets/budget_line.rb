...
module GobiertoBudgets
  class BudgetLine < OpenStruct
    ...
    def self.first
      ...
      area = if @conditions[:area_name] == GobiertoBudgets::BudgetLine::ECONOMIC
               GobiertoBudgets::EconomicArea
             else
               GobiertoBudgets::FunctionalArea
             end
    ...
    def self.all
      ...
      area = if @conditions[:area_name] == ECONOMIC
               GobiertoBudgets::EconomicArea
             else
               GobiertoBudgets::FunctionalArea
             end
      ...
      response['hits']['hits'].map { |h| h['_source'] }.map do |row|
        BudgetLinePresenter.new(row.merge(kind: @conditions[:kind], area_name: @conditions[:area_name], area: area, total: response['aggregations']['total_budget']['value'],
    ...
    def self.budget_line_query(options)
      ...
      if population_filter && (population_filter[:from].to_i > GobiertoBudgets::Population::FILTER_MIN || population_filter[:to].to_i < GobiertoBudgets::Population::FILTER_MAX)
        ...
        reduced_filter[:aarr] = aarr_filter if aarr_filter
      ...
      terms << { term: { autonomy_id: aarr_filter } } if aarr_filter.present?
      ...
      query[:_source] = false if options[:to_rank]
    ...
    def self.find(options)
      search(options)['hits'].detect { |h| h['code'] == options[:code] }
    end
    ...
    def self.top_differences(options)
      ...
      [results.sort { |b, a| a[1][2] <=> b[1][2] }[0..15], results.sort { |a, b| a[1][2] <=> b[1][2] }[0..15]]
    end
    ...
    def place
      INE::Places::Place.find(place_id) if place_id
    end
