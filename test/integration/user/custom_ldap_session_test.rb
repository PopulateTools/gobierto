...
class User::CustomSessionTest < ActionDispatch::IntegrationTest
  ...
  def test_site_custom_session_with_ldap_strategy_valid_not_existing_user
    with(site: site) do
  ...
  def test_site_custom_session_with_ldap_strategy_valid_existing_user
  ...
  def test_site_custom_session_invalid_user
    with(site: site) do
      visit new_user_sessions_path
