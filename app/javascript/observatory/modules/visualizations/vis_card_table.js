...
export var TableCard = Class.extend({
  init: function(divClass, json, nest, cardName) {
    ...
    this.div.selectAll('.tw-sharer')
      .attr('target', '_blank')
      .attr('href', 'https://twitter.com/intent/tweet?text=' + I18n.t('gobierto_observatory.cards.meta.where') + encodeURI(window.populateData.municipalityName) + ': ' +  encodeURI(I18n.t('gobierto_observatory.cards.' + cardName + '.title')).toLowerCase() + I18n.t('gobierto_observatory.cards.meta.time') + encodeURI(formatDate(parsedDate).toLowerCase()) + ', ' + encodeURI(this._printData(nest[0].value.valueOne))  + '&url=' + window.location.href + '&via=gobierto&source=webclient');
    ...
    var header = nest.map(function(d) {
      return I18n.t('gobierto_observatory.cards.' + cardName + '.' + this._normalize(d.value.column));
    }.bind(this));
