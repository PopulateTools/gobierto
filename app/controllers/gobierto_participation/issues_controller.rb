...
module GobiertoParticipation
  class IssuesController < GobiertoParticipation::ApplicationController
    ...
    def find_issue_notifications
      ActivityCollectionDecorator.new(Activity.in_site(current_site).no_admin.in_process(@issue.processes.open_process).sorted.limit(5).includes(:subject, :author, :recipient))
    end
