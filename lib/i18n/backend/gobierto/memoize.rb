    ...
    module GobiertoCore
      module Memoize
        ...
        def lookup(locale, key, scope = nil, options = {})
          flat_key  = I18n::Backend::Flatten.normalize_flat_keys(locale, key, scope, options[:separator]).to_sym
          flat_hash = memoized_lookup[locale.to_sym]
          flat_hash.key?(flat_key) ? flat_hash[flat_key] : (flat_hash[flat_key] = super)
        end

        def memoized_lookup
          site_id = ::GobiertoCore::CurrentScope.current_site.try(:id) || 0
          memoized_lookups[site_id] ||= I18n.new_double_nested_cache
