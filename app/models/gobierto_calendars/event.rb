  ...
  class Event < ApplicationRecord
    ...
    def to_url(options = {})
      if collection.container_type == "GobiertoParticipation::Process"
        url_helpers.gobierto_participation_process_event_path(id: slug, process_id: collection.container.slug, host: app_host)
      elsif collection.container_type == "GobiertoParticipation"
        url_helpers.gobierto_participation_event_path(id: slug, host: app_host)
      else
        url_helpers.gobierto_people_person_event_path(parameterize.merge(host: app_host).merge(options))
      end
    end
