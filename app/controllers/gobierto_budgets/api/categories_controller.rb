  ...
  module Api
    class CategoriesController < ApplicationController
      ...
      def index
        kind = params[:kind]
        area = params[:area]
        render_404 && return if area == 'functional' && kind == 'I'
        ...
        if kind.nil? && area.nil?
          ...
          [GobiertoBudgets::EconomicArea, GobiertoBudgets::FunctionalArea].each do |klass|
            ...
            [GobiertoBudgets::BudgetLine::INCOME, GobiertoBudgets::BudgetLine::EXPENSE].each do |kind|
              next if kind == GobiertoBudgets::BudgetLine::INCOME && klass == GobiertoBudgets::FunctionalArea

              categories[area_name] ||= {}
              categories[area_name][kind] = Hash[klass.all_items[kind].sort_by { |k, _v| k.to_f }]
            end
          end
        else
          klass = area == 'economic' ? GobiertoBudgets::EconomicArea : GobiertoBudgets::FunctionalArea
          categories = Hash[klass.all_items[kind].sort_by { |k, _v| k.to_f }]
        end
