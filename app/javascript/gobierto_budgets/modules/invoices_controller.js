...
window.GobiertoBudgets.InvoicesController = (function() {
    ...
    d3
      ...
      .get(function(error, csv) {
        ...
        if (!data.length) {
          ...
          let previousFilter =
            filters.indexOf(filter) > 0 ? filters[filters.indexOf(filter) - 1] : alert(I18n.t("gobierto_budgets.invoices.show.empty"));
  ...
  function _renderByMonthsChart() {
    ...
    bars
      ...
      ) // webkit doesn't recalculate dynamic width. it has to be set by parentNode
      .dimension(months)
      .group(budgetMonthly)
      .x(d3.scaleBand())
      ...
      .barPadding(0.45)
      .title(function(d) {
        return _abbrevLargeCurrency(d.value, { minimumFractionDigits: 0 });
      })
    ...
    bars.yAxis().ticks(5);
    bars.yAxis().tickFormat(function(v) {
  ...
  function _renderMainProvidersChart() {
    ...
    hbars1
      ...
      .title(function(d) {
        return d.key + " - " + _abbrevLargeCurrency(d.value, { minimumFractionDigits: 0 });
      })
      .elasticX(true)
      .on("pretransition", function(chart) {
        // Apply rounded corners AFTER render, otherwise they don't exist
  ...
  function _renderByAmountsChart() {
          ...
          .data(d => {
            ...
            function intervalFormat(d) {
              ...
              if (d.key === _r.domain.length) {
                return [
                  I18n.t("gobierto_budgets.invoices.show.more") +
                    " " +
    ...
    hbars2.xAxis().tickFormat(function(tick, pos) {
      ...
      controller: {
        loadData: function(filter) {
          return $.grep(_tabledata || data, function(row) {
            return (
              (!filter.provider_name.toLowerCase() || row.provider_name.toLowerCase().indexOf(filter.provider_name.toLowerCase()) > -1) &&
