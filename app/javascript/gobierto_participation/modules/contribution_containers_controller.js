...
window.GobiertoParticipation.ContributionContainersController = (function() {
  ...
  ContributionContainersController.prototype.show = function(
    contribution_container_data
  ) {
  ...
  function _initContributionContainer(contribution_container_data) {
    ...
    var users_worst_ratings =
      contribution_container_data["users_worst_ratings"];
    ...
    contributions
      ...
      .attr(
        "class",
        "fas fa-angle-left fa-3x arrow previous_contribution disabled"
      )
      .on("click", updateContributions);
    ...
    if (urlHash.length) {
      var selected_cards = data.filter(function(c) {
        return c.slug == urlHash.substr(1);
      });
    ...
    $("#next").click(function() {
      ...
      createCards(nodes);
      if (page === page_max) {
        d3.select(".next_contribution").classed("disabled", true);
      }
    ...
    /* Buttons */
    contributions.select(".toggleLayout").on("click", updateLayout);

    contributions.selectAll(".cardControl").on("click", updateCards);
    ...
    function updateCards() {
      ...
      if (page_max > 1) {
        d3.select(".next_contribution").classed("disabled", false);
      }
    ...
    function updateContributions() {
      ...
      d3.select(".circles_progress .active").classed("trackToggle", true);

      if (toggle === "next") {
        d3.selectAll(".circles_progress .active + li").classed(
          "active",
          "true"
        );

        d3.select(".previous_contribution").classed("disabled", false);
      } else {
    ...
    function ticked() {
      ...
      d3.selectAll(".card").style("transform", function(d) {
        return (
          "translate(" +
          d.x +
          "px," +
    ...
    function dragged(event, d) {
      vx = event.x - px;
      vy = event.y - py;
      d.fx = Math.max(Math.min((px = event.x) - offsetX, width - d.width), 0);
      d.fy = Math.max(Math.min((py = event.y) - offsetY, height - d.height), 0);
    ...
    function ended(d) {
      ...
      var vScalingFactor =
        maxVelocity / Math.max(Math.sqrt(vx * vx + vy * vy), maxVelocity);
    ...
    function createPagination(page_max) {
      for (var i = 1; i <= page_max; i++) {
        if (i === 1) {
          $(".circles_progress").append(
            "<li data-page=" +
              i +
              ' class="active"><a href=""><span></span></a></li>'
          );
        } else {
          $(".circles_progress").append(
            "<li data-page=" + i + '><a href=""><span></span></a></li>'
          );
        }
      }
    }
    ...
    function checkPreviousNextPagination(page, page_max) {
      ...
      if (page == 1) {
        d3.select(".previous_contribution").classed("disabled", true);
      }
    }
    ...
    function createNodes(data) {
      ...
      output.forEach(function(d) {
        (d.total = d.name["votes_count"]), (d.love_pct = d.name["love_pct"]);
    ...
    function createCards(data) {
      ...
      card
        ...
        .style("background", function(d) {
          ...
          return (
            "linear-gradient(to right, #38b486 " +
            d.name["love_pct"] +
            "%, #429aad " +
            (d.name["love_pct"] + d.name["like_pct"]) +
            "%, #9c7e7e " +
            (d.name["love_pct"] + d.name["like_pct"] + d.name["neutral_pct"]) +
            "%, #f06659 " +
            (d.name["love_pct"] +
              d.name["like_pct"] +
              d.name["neutral_pct"] +
