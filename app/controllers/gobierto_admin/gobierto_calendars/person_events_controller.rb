  ...
  module GobiertoPeople
    module People
      class PersonEventsController < People::BaseController
        ...
        def new
          @person_event_form = PersonEventForm.new(person_id: @person.id)
        ...
        def edit
          ...
          @person_event_form = PersonEventForm.new(
            @person_event.attributes.except(*ignored_person_event_attributes).merge(person_id: @person.id)
          )
          @attendees = get_attendees
        end
        ...
        def update
          @person_event = find_person_event
          @person_event_form = PersonEventForm.new(
            person_event_params.merge(id: params[:id], admin_id: current_admin.id, site_id: current_site.id, person_id: @person.id)
          )
